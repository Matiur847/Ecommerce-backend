...
exports.loginUser = async (req, res) => {
    ...
    if (!email || !password) {
        return res.send('Please enter your email & password')
    }
    if (!user) {
...
exports.forgotPassword = async (req, res, next) => {
    ...
    try {
        ...
        const resetPassworUrl = `${req.protocol}://${req.get("host")}/api/v1/password/reset/${resetToken}`
...
exports.updateUserPassword = async (req, res, next) => {
    ...
    const { token } = req.cookies
    const decodeData = jwt.verify(token, jwtSec)
    const user = await User.findById(decodeData.id).select("password").select("+password")
